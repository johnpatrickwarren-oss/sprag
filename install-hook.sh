#!/bin/bash
# install-hook.sh <repo-dir> <src-rel-path>
...
# so rot can't land. Bypass (discouraged): git commit --no-verify.
set -euo pipefail
REPO="${1:?usage: install-hook.sh <repo-dir> <src-rel-path>}"
SRC="${2:?usage: install-hook.sh <repo-dir> <src-rel-path>}"
GATE="$(cd "$(dirname "$0")" && pwd)/arch-gate.mjs"
HOOK="$REPO/.git/hooks/pre-commit"
mkdir -p "$(dirname "$HOOK")"

# Unquoted heredoc: $GATE/$SRC are baked in now; \$runtime vars stay literal for the hook.
cat > "$HOOK" <<EOF
#!/bin/bash
...
if git rev-parse --verify -q HEAD >/dev/null; then
  ...
  if [ -d "\$tmp/\$ARCH_SRC" ]; then
    node "\$ARCH_GATE" "\$tmp/\$ARCH_SRC" --baseline --baseline-out "\$tmp/baseline.json" >/dev/null 2>&1 || true
  fi
fi
if ! node "\$ARCH_GATE" "\$repo/\$ARCH_SRC" --baseline-in "\$tmp/baseline.json"; then
  echo ""
...
EOF
chmod +x "$HOOK"
echo "installed pre-commit gate -> $HOOK   (watching '$SRC', gate=$GATE)"
