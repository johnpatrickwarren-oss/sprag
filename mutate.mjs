...
//
//   arch mutate <dir> --test "<cmd>" [--since <ref>] [--all] [--threshold 60] [--max-mutants 200] [--cwd <d>]
//
...
const TEST = opt('--test');
if (!dir || !TEST) { console.error('usage: arch mutate <dir> --test "<cmd>" [--since <ref>] [--all] [--threshold 60] [--max-mutants 200] [--cwd <d>]'); process.exit(64); }
...
let targets = [];
if (has('--all')) {
  (function walk(d) { for (const n of readdirSync(d)) { if (['node_modules', '.git', 'vendor'].includes(n)) continue; const p = join(d, n); if (statSync(p).isDirectory()) walk(p); else if (SRC.some((e) => n.endsWith(e)) && !isTest(n) && !isGen(p)) targets.push(p); } })(dir);
} else {
  ...
  targets = (r.stdout || '').split('\n').filter(Boolean).map((f) => join(repoRoot, f))
    .filter((p) => p.startsWith(dir) && existsSync(p) && SRC.some((e) => p.endsWith(e)) && !isTest(p) && !isGen(p));
}
